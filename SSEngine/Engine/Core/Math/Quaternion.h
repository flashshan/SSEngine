...
public:
	...
	FORCEINLINE Quaternion(const Rotator &i_other);
	FORCEINLINE Quaternion(const Quaternion &i_other);


	FORCEINLINE Quaternion& operator=(const Quaternion &i_other);

	FORCEINLINE bool Equal(const Quaternion& i_other, float i_tolerance = SMALL_NUMBER);

	FORCEINLINE Quaternion operator +(const Quaternion &i_other) const;
	FORCEINLINE void operator +=(const Quaternion &i_other);
	FORCEINLINE Quaternion operator -(const Quaternion &i_other) const;
	FORCEINLINE void operator -=(const Quaternion &i_other);
	
	FORCEINLINE Quaternion operator *(const Quaternion &i_other) const;
	FORCEINLINE void operator *=(const Quaternion &i_other);
	FORCEINLINE Quaternion operator *(const float i_float) const;
